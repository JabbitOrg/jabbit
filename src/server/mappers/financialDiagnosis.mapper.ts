...
export class FinancialDiagnosisMapper {
  static getColumnIndexes(headerRow: string[]) {
    return FINANCIAL_DIAGNOSIS_SHEET_HEADERS.reduce((acc, header) => {
  ...
  static fromSheetRow(headerRow: string[], dataRow: string[]): FinancialDiagnosis {
    ...
    return {
      ...
      mainInvestmentStrategies: JSON.parse(dataRow[columnIndexes.mainInvestmentStrategies]),
      mainFinancialIssues: JSON.parse(dataRow[columnIndexes.mainFinancialIssues]),
